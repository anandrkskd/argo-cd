...
func SetAppSpecOptions(flags *pflag.FlagSet, spec *argoappv1.ApplicationSpec, appOpts *AppOptions, sourcePosition int) int {
		...
		case "sync-policy":
			...
			case "none", "manual":
				if spec.SyncPolicy != nil {
					spec.SyncPolicy.Automated.Enable = false
				}
			...
			case "automated", "automatic", "auto":
				...
				spec.SyncPolicy.Automated = argoappv1.SyncPolicyAutomated{Enable: true}
			default:
	...
	if flags.Changed("auto-prune") {
		if spec.SyncPolicy == nil || spec.SyncPolicy.Automated.Enable == false {
